...
def compute_zs(
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        weight_decay_mlp = hparams.v_weight_decay * (torch.norm(delta_mlp) / torch.norm(target_init_mlp) ** 2) 
        weight_decay_attn = hparams.v_weight_decay_attn *(torch.norm(delta_attn) / torch.norm(target_init_attn) ** 2)
        # weight_decay = hparams.v_weight_decay * torch.norm(delta) ** 2
        loss = nll_loss + kl_loss + weight_decay_mlp+ weight_decay_attn
        prob = torch.exp(-nll_loss_each).mean().item()
        print(
            f"loss {np.round(loss.item(), 3)} = {np.round(nll_loss.item(), 3)} + {np.round(kl_loss.item(), 3)} + {np.round(weight_decay_mlp.item(), 3)} + {np.round(weight_decay_attn.item(), 3)} "
