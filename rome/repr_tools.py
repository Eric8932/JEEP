...
def get_words_idxs_in_templates(
    ...
    if 'gpt-j' not in tok.name_or_path:
        for i in range(len(words_len)):
            if words[i].strip()=="Āryabhaṭīya":
                words_len[i]+=1
    if subtoken == "last" or subtoken == "first_after_last":
        if 'gpt-j' not in tok.name_or_path:
            return [ [prefixes_len[i]+ words_len[i]-3 + (1 if prefixes_len[i] ==1 else 0)+ (1 if words[i] ==" "  else 0)
                     +(1 if len(words[i])>1 and words[i][1] in [str(number) for number in range(10)]+['Ō','İ',"Ş","Ç","Đ","Æ"] and prefixes_len[i] !=1  else 0 )
                     ] for i in range(n)
                   ]
        else:
            return [ [ prefixes_len[i]+ words_len[i] -1+ (1 if words[i] =="" else 0) ] for i in range(n) ]
    
    elif subtoken == "first":
        return [[prefixes_len[i]] for i in range(n)]
    elif subtoken == "pred_last" :
        if 'gpt-j' not in tok.name_or_path:
            return [[prefixes_len[i]+words_len[i]+suffixes_len[i] -3 
                     + (1 if (words[i] ==" " and len(suffixes[i])>0 and suffixes[i][0] not in [",","?"]) or words[i]=="" else 0) 
                     -(0 if prefixes_len[i] ==1 else 1)  - (0 if suffixes_len[i] == 1 else 1)+(1 if len(suffixes[i])>0 and suffixes[i][0] in punctuation and (len(words[i])>0 and words[i][-1] not in punctuation and words[i][-1] not in ["”"]) else 0)
                    +(1 if len(words[i])>1 and words[i][1] in [str(number) for number in range(10)] and prefixes_len[i] !=1  else 0 )
                     +(1 if (len(words[i])>1 and words[i][-1] in ["."]  and words[i][-2] in ["."] and words[i][-3] not in ["."] and len(suffixes[i])>0 and suffixes[i][0] in [",","?"]) or (len(words[i])>1 and words[i][1] in ['Ō','İ',"Ş","Ç","Đ","Æ"]) else 0)
                     +(1 if (len(words[i])>0 and words[i][-1] in [".","!"] and len(suffixes[i])>0 and suffixes[i][0] in ["?"]) else 0)
                    ]   
                    for i in range(n)]
        else:
            return [[prefixes_len[i]+words_len[i]+suffixes_len[i]-1 
                     -(1 if words[i]==" " and len(suffixes[i])>0 and suffixes[i][0] in [",","?"] else 0)
                    -(1 if len(words[i])>0 and words[i][-1] in [")","!","?",".","'",'"'] and words[i][-2] not in ["."] and suffixes[i][0] in [",","?"] else 0 )]
                      for i in range(n)]
    else:
...
def get_reprs_at_idxs(
    ...
    for batch_contexts, batch_idxs in _batch(n=64):
        ...
        if tin:
            if module_name in ["transformer.h.8","transformer.h.16","transformer.h.21","transformer.h.23","transformer.h.19","transformer.h.17"] :
