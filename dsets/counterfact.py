...
class CounterFactDataset(Dataset):
    def __init__(
        ...
        if num_edits is not None:
            cf_loc = "mcf_seq_edit{}".format(num_edits)+".json"
        if not cf_loc.exists():
            remote_url = f"{REMOTE_ROOT}/{'multi_' if multi else ''}counterfact"
...
class MultiCounterFactDataset(CounterFactDataset):
    def __init__(
        self, data_dir: str, size: typing.Optional[int] = None,tok=None, *args, **kwargs
    ):
